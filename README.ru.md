...
**v5.2** — заставляет *доказать* — **чиня и перезапуская проверки до `exit 0`**. Не «готово» на red. Не сдаваться.
...
- **Fix-until-green** — red → чини → re-run → снова, пока не `exit 0`; задачу пользователю не бросаешь
- **3-strike** — тот же патч 3 раза → другой дизайн, потом **дальше чинить** до green
...
| «Пиши чисто» | «Чини до green — потом докажи evidence» |
...
```text
  ...
  │  Агент не может «вроде ок» — red → цикл fix → re-run → только exit 0.    │
